...
@router.get("/", response_model=GenericResponse[PaginatedErrorLogResponse]) # Updated response_model
async def list_error_logs(
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(10, ge=1, le=100, description="Number of items per page"),
    service_name: Optional[str] = Query(None, description="Filter by service name"),
    error_type: Optional[str] = Query(None, description="Filter by error type"),
    sort_field: str = Query("timestamp", description="Field to sort by (e.g., 'timestamp', 'service_name', 'error_type')"),
    sort_order: str = Query("desc", description="Sort order: 'asc' or 'desc'"),
    mongo_service: MongoService = Depends(get_mongo_service)
):
    try:
        logfire.info(f"API: Received request to list error logs. Page: {page}, PageSize: {page_size}, Service: {service_name}, Type: {error_type}")
        ...
        logfire.error(f"API: Unexpected error listing error logs: {e}", exc_info=True)
