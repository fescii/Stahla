...
async def _perform_blandai_check(bland_ai_manager: BlandAIManager) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(bland_status_dict, dict) and bland_status_dict.get("status") == "success":
            logfire.debug("Health check: BlandAI connection status: ok", details=bland_status_dict.get("details"))
            return check_name, {"status": "ok", "message": bland_status_dict.get("message")}
        elif isinstance(bland_status_dict, dict):
            error_message = bland_status_dict.get("message", "Unknown error from Bland AI check")
            logfire.warning(f"Health check: BlandAI connection status: error, Message: {error_message}", details=bland_status_dict.get("details"))
            ...
            logfire.error(f"Health check: BlandAI connection check returned unexpected type: {type(bland_status_dict)}", response=bland_status_dict)
    ...
    try:
        ...
        if redis_client:
            try:
                ...
                logfire.debug("Health check: Redis client connection closed within helper.")
            except Exception as e_close:
                logfire.error(f'Health check: Error closing Redis client: {str(e_close)}', exc_info=True)
