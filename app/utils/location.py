...
def geocode_location(
    ...
    if state_code:
        try:
            ...
            if location:
                logfire.info(
                    f"State-enhanced geocoding successful: Found ({location.latitude}, {location.longitude})"
                )
                return location.latitude, location.longitude
        except (GeocoderTimedOut, GeocoderServiceError, Exception) as e:
    ...
    for context in common_contexts:
        ...
        try:
            ...
            if location:
                logfire.info(
                    f"Enhanced geocoding successful: Found ({location.latitude}, {location.longitude})"
                )
                return location.latitude, location.longitude
        except (GeocoderTimedOut, GeocoderServiceError, Exception) as e:
            logfire.warn(
    ...
    try:
        ...
        location = geolocator.geocode(
            {"q": place_terms}, timeout=10, exactly_one=False, limit=1
        )

        if location and len(location) > 0:
            first_match = location[0]
            logfire.info(
                f"Structured geocoding successful: Found ({first_match.latitude}, {first_match.longitude})"
            )
    ...
    # All attempts failed
    logfire.warn(
        f"Geocoding failed: No location found for '{location_description}'",
        state_code=state_code,
    )
    return None, None
