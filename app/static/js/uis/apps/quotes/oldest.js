export default class QuotesOldest extends HTMLElement {
  constructor() {
    super();
    this.shadowObj = this.attachShadow({ mode: "open" });
    this.app = window.app;
    this.api = this.app.api;
    this.url = this.getAttribute("api") || "/mongo/quotes/oldest";
  ...
  fetchQuotes = async (page = 1) => {
    ...
    try {
      ...
      console.error("Error fetching oldest quotes:", error);
      if (error.status === 401) {
        this._loading = false;
        this.app.showLogin();
        return;
  ...
  getHeader = () => {
    return /* html */ `
      <div class="header">
        <h1>Oldest Quotes</h1>
        <p class="subtitle">Quotes ordered from oldest to newest</p>
      </div>
    `;
  };
  ...
  getQuoteCard = (quote) => {
    return /* html */ `
      <div class="quote-card vintage" data-quote-id="${quote.id}" tabindex="0">
        ...
        <div class="quote-details">
          <div class="detail-row">
            <div class="detail-item">
              ...
              <span class="detail-value amount">${this.formatCurrency(quote.total_amount)}</span>
            </div>
            <div class="detail-item">
          ...
          <div class="quote-footer">
            <span class="timestamp vintage-badge">Created: ${this.formatDate(quote.created_at)}</span>
  ...
  getEmptyMessage = () => {
    return /* html */ `
      <div class="empty-state">
        <h2>No Historic Quotes</h2>
        <p>There are no historic quotes to display at this time.</p>
      </div>
    `;
  };
    ...
    return /* html */ `
      <style>
        ...
        * {
        ...
        .quote-card.vintage {
          border-left: 4px solid var(--second-linear);
          background: linear-gradient(135deg, var(--stat-background) 0%, rgba(223, 121, 26, 0.05) 100%);
        }
        ...
        .quote-card.vintage:hover {
          background: linear-gradient(135deg, var(--hover-background) 0%, rgba(223, 121, 26, 0.08) 100%);
        }
        ...
        .vintage-badge {
          background: linear-gradient(90deg, rgba(223, 121, 26, 0.1) 0%, rgba(240, 156, 78, 0.1) 100%);
