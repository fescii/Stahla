export default class QuotesRecent extends HTMLElement {
  constructor() {
    super();
    this.shadowObj = this.attachShadow({ mode: "open" });
    this.app = window.app;
    this.api = this.app.api;
    this.url = this.getAttribute("api") || "/mongo/quotes/recent";
  ...
  getSVGIcon = (type) => {
    const icons = {
      ...
      recent: `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  getHeader = () => {
    return /* html */ `
      <div class="header">
        <h1>Recent Quotes</h1>
        <p class="subtitle">Latest quote requests ordered by creation date</p>
      </div>
    `;
  };
  ...
  getQuoteRow = (quote, index) => {
    ...
    const recentBadge = this.getRecentBadge(quote.created_at);
  ...
  getRecentBadge = (dateString) => {
    ...
    if (diffMinutes <= 30) {
      return /* html */ `
        <div class="recent-badge fresh">
          ${this.getSVGIcon('recent')}
          <span class="recent-text">Fresh</span>
        </div>
      `;
    } else if (diffHours <= 2) {
      return /* html */ `
        <div class="recent-badge hot">
          ${this.getSVGIcon('recent')}
          <span class="recent-text">Hot</span>
        </div>
      `;
    } else if (diffHours <= 24) {
      return /* html */ `
        <div class="recent-badge today">
          ${this.getSVGIcon('recent')}
          <span class="recent-text">Today</span>
        </div>
      `;
    } else if (diffDays <= 7) {
      return /* html */ `
        <div class="recent-badge week">
          ${this.getSVGIcon('recent')}
          <span class="recent-text">This Week</span>
        </div>
      `;
    }
  ...
  getEmptyMessage = () => {
    return /* html */ `
      <div class="empty-state">
        <h2>No Recent Quotes</h2>
        <p>There are no recent quotes to display at this time.</p>
      </div>
    `;
  };
