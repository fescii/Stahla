export default class QuotesHighest extends HTMLElement {
  ...
  fetchQuotes = async (page = 1) => {
    ...
    try {
      ...
      console.error("Error fetching highest quotes:", error);
      if (error.status === 401) {
        this._loading = false;
        this.app.showLogin();
        return;
  ...
  getSVGIcon = (type) => {
    const icons = {
      ...
      rank1: `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
      </svg>`,
      rank2: `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <circle cx="12" cy="12" r="10"/>
        <path d="M8 14s1.5 2 4 2 4-2 4-2"/>
        <line x1="9" y1="9" x2="9.01" y2="9"/>
        <line x1="15" y1="9" x2="15.01" y2="9"/>
      </svg>`,
      rank3: `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <circle cx="12" cy="12" r="10"/>
        <path d="M8 12h8"/>
        <path d="M8 16h8"/>
        <path d="M8 8h8"/>
  ...
  getRankBadge = (rank) => {
    const badges = {
      1: { icon: 'rank1', text: '#1', class: 'gold' },
      2: { icon: 'rank2', text: '#2', class: 'silver' },
      3: { icon: 'rank3', text: '#3', class: 'bronze' }
    };
