...
class PricingInput(BaseModel):
    ...
    product_type: Optional[str] = Field(None, description="Primary product type requested (e.g., 'Restroom Trailer', 'Portable Toilet'). Derived from Slot: Product_Type.")
    num_units: Optional[int] = Field(None, description="Number of units or stalls requested. Derived from Slot: Units_Needed.")
    usage_type: Optional[Literal["event", "construction", "facility", "disaster_relief", "other"]] = Field(None, description="The intended use case for the rental. Derived from Slot: Customer_Type.")
    rental_duration_days: Optional[int] = Field(None, description="Total number of days the rental is needed. Derived from Slot: Duration.")
    ...
    delivery_location_description: Optional[str] = Field(None, description="Delivery address or city/state. Derived from Slot: Location.")
    is_local: Optional[bool] = Field(None, description="Whether the location is considered 'local' based on proximity to hubs. Determined internally or from call script.")
    site_surface: Optional[Literal["cement", "gravel", "dirt", "grass", "asphalt", "concrete"]] = Field(None, description="Surface type at the placement location. Derived from Slot: Surface_Type.")
    ground_level: Optional[bool] = Field(None, description="Is the ground level/flat? Derived from Slot: Ground_Level.")
    obstacles_present: Optional[bool] = Field(None, description="Are there low branches or obstacles? Derived from Slot: Obstacles.")
    power_available: Optional[bool] = Field(None, description="Is adequate power available at the placement site? Derived from Slot: Power_Available.")
    water_available: Optional[bool] = Field(None, description="Is a water hookup (garden hose) available at the placement site? Derived from Slot: Water_Available.")
    ...
    requires_ada: Optional[bool] = Field(None, description="Does the request specifically require ADA-compliant units? Derived from Slot: ADA_Required.")
    requires_shower: Optional[bool] = Field(None, description="Are shower facilities required? Derived from Slot: Shower_Required.")
    requires_handwashing: Optional[bool] = Field(None, description="Are separate handwashing stations required? Derived from Slot: Handwashing_Needed.")
    requires_cleaning_service: Optional[bool] = Field(None, description="Is regular cleaning/restocking service requested? Derived from Subflow SB.")
    ...
    other_products_mentioned: List[str] = Field(default_factory=list, description="Other products mentioned (e.g., ['Tent', 'Generator']). Derived from Subflow SA/SB.")
...
class PriceQuote(BaseModel):
    ...
    request_id: str = Field(..., description="Identifier linking back to the PricingInput request.")
    quote_id: str = Field(default_factory=lambda: f"QT-{uuid.uuid4()}", description="Unique identifier for this generated quote.")
    line_items: List[QuoteLineItem] = Field(default_factory=list, description="List of itemized charges.")
    total_amount: float = Field(..., description="The total estimated price.")
    currency: str = Field(default="USD", description="Currency code.")
    notes: Optional[str] = Field(None, description="Additional notes, disclaimers, or explanations about the quote.")
    is_estimate: bool = Field(default=True, description="Indicates if this is a preliminary estimate or a firm quote.")
    missing_info: List[str] = Field(default_factory=list, description="List of critical information missing that prevented a firm quote.")
    generated_at: str = Field(default_factory=lambda: datetime.utcnow().isoformat(), description="Timestamp when the quote was generated.")
