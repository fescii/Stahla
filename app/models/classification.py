...
class ClassificationInput(BaseModel):
    ...
    is_in_service_area: Optional[bool] = Field(None, description="Whether the delivery address is within the defined service area.")

    # --- Fields based on PRD & Call Script (including call.json variables) ---
    # Contact Info (often extracted)
    firstname: Optional[str] = Field(None, description="Prospect's first name (from metadata)")
    lastname: Optional[str] = Field(None, description="Prospect's last name (from metadata)")
    email: Optional[EmailStr] = Field(None, description="Prospect's email address (from metadata)")
    phone: Optional[str] = Field(None, description="Prospect's phone number (from metadata)")
    company: Optional[str] = Field(None, description="Prospect's company name (from metadata)")
    # Added based on call.json variables
    contact_name: Optional[str] = Field(None, description="Verified or collected contact name during the call.")
    company_name: Optional[str] = Field(None, description="Verified or collected company name during the call.")
    contact_email: Optional[EmailStr] = Field(None, description="Verified or collected contact email during the call.")
    ...
    # Added based on call.json variables
    what_service_do_you_need_: Optional[str] = Field(None, description="Initial service interest from metadata.")
    product_type_interest: Optional[str] = Field(None, description="Primary product type confirmed/collected during call.")
    units_needed: Optional[str] = Field(None, description="Number of units/stalls needed or expected attendance (as string).")
    how_many_portable_toilet_stalls_: Optional[int] = Field(None, description="Initial number of porta potty stalls from metadata.")
    required_stalls: Optional[int] = Field(None, description="Parsed number of stalls or units needed.") # Keep existing for potential parsing
    ada_required: Optional[bool] = Field(None, description="Is an ADA-compliant unit required?")
    # Added based on call.json variables
    shower_required: Optional[bool] = Field(None, description="Is a unit with a shower required?")
    handwashing_needed: Optional[bool] = Field(None, description="Are handwashing stations or sinks needed?")
    additional_services_needed: Optional[str] = Field(None, description="Other services mentioned (e.g., holding tanks, servicing).")
    ...
    # Added based on call.json variables
    project_category: Optional[str] = Field(None, description="Category of use confirmed/collected during call (e.g., Construction, Special Event).")
    event_location_description: Optional[str] = Field(None, description="Delivery address or general location description (from metadata)")
    # Added based on call.json variables
    service_address: Optional[str] = Field(None, description="Full delivery address confirmed/collected during call.")
    event_state: Optional[str] = Field(None, description="Two-letter state code where the event will take place (e.g., 'NY', 'NE')")
    # Added based on call.json variables
    state: Optional[str] = Field(None, description="State extracted from service_address.")
    ...
    # Added based on call.json variables
    address_type: Optional[str] = Field(None, description="Type of delivery location (e.g., residence, business, event venue).")
    event_location_type: Optional[Literal["business", "residence", "other"]] = Field(None, description="Type of delivery location") # Keep existing for potential mapping
    delivery_surface: Optional[Literal["cement", "gravel", "dirt", "grass"]] = Field(None, description="Surface type at placement location") # Keep existing for potential mapping
    # Added based on call.json variables
    site_ground_level: Optional[bool] = Field(None, description="Will the unit be placed on level ground?")
    site_ground_type: Optional[str] = Field(None, description="Surface type confirmed/collected during call (e.g., grass, gravel).")
    delivery_obstacles: Optional[str] = Field(None, description="Description of potential delivery obstacles (e.g., low trees)")
    # Added based on call.json variables
    site_obstacles: Optional[str] = Field(None, description="Obstacles confirmed/collected during call.")
    ...
    start_date: Optional[str] = Field(None, description="Ideal delivery/start date (from metadata)")
    end_date: Optional[str] = Field(None, description="Ideal pickup/end date (from metadata)")
    # Added based on call.json variables
    rental_start_date: Optional[str] = Field(None, description="Rental start date confirmed/collected during call.")
    rental_end_date: Optional[str] = Field(None, description="Rental end date confirmed/collected during call.")
    guest_count: Optional[int] = Field(None, description="Estimated number of attendees/users")
    # Added based on call.json variables
    expected_attendance: Optional[int] = Field(None, description="Expected attendance confirmed/collected during call.")
    ...
    # Added based on call.json variables
    power_source_distance: Optional[str] = Field(None, description="Distance to power source confirmed/collected during call (as string).")
    power_path_cross: Optional[bool] = Field(None, description="Does the power cord need to cross a path?")
    power_cord_ramps_needed: Optional[bool] = Field(None, description="Are cord ramps needed for power?") # Keep existing
    generator_needed: Optional[bool] = Field(None, description="Is a generator needed?") # Keep existing
    ...
    # Added based on call.json variables
    water_source_distance: Optional[str] = Field(None, description="Distance to water source confirmed/collected during call (as string).")
    water_path_cross: Optional[bool] = Field(None, description="Does the water hose need to cross a path?")
    water_hose_ramps_needed: Optional[bool] = Field(None, description="Are hose ramps needed for water?") # Keep existing

    # Consent & Follow-up
    # Added based on call.json variables
    recording_consent_given: Optional[bool] = Field(None, description="Was consent given for recording (implicitly)?")
    contact_consent_given: Optional[bool] = Field(None, description="Was explicit consent given for contact/quote?")
    by_submitting_this_form_you_consent_to_receive_texts: Optional[bool] = Field(None, description="Consent to receive texts from metadata.")
    follow_up_call_scheduled: Optional[bool] = Field(None, description="Was a follow-up call requested/scheduled?")
    referral_accepted: Optional[bool] = Field(None, description="Did the user accept the offer for a referral (if OOS)?")
    ...
    # Added based on call.json variables
    decision_timing: Optional[str] = Field(None, description="Decision timing confirmed/collected during call.")
    quote_needed_by: Optional[str] = Field(None, description="How soon is the quote needed?")
    # Added based on call.json variables
    quote_urgency: Optional[str] = Field(None, description="Quote urgency confirmed/collected during call.")
