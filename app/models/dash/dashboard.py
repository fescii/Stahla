...
class CacheSearchResult(BaseModel):
    ...
    value_preview: Optional[Any] = Field(None, description="Preview of the cached value (parsed JSON subset or truncated string).")
    ttl: Optional[int] = None

class CacheHitMissRatio(BaseModel):
    percentage: Optional[float] = Field(None, description="Cache hit percentage (0.0 to 1.0).")
    ...
    status: str = Field("Not Tracked", description="Overall status message, e.g., Not Tracked, N/A (No data).")
...
class SyncStatus(BaseModel):
    ...
    recent_sync_errors: List[str] = Field(default_factory=list, description="List of recent sync error messages.")
...
class SheetProductEntry(BaseModel):
    ...
    extras: Optional[Dict[str, Optional[float]]] = Field(default_factory=dict, description="Extra service costs like pump_out, fresh_water_fill, etc.")
...
class SheetConfigEntry(BaseModel):
    id: str = Field(..., alias="_id", description="Document ID in MongoDB, e.g., 'master_config'.") # Alias for _id
