...
class SheetSyncService:
    ...
    async def initialize_service(self):
        ...
        try:
            self.sheet_service = await loop.run_in_executor(
                None, create_sheets_service, settings.GOOGLE_APPLICATION_CREDENTIALS
            )
            logfire.info(
    ...
    def _parse_delivery_and_config(
        ...
        if not config_data or len(config_data) < 2:
            logfire.warning(
                f"No data or insufficient rows in config_data for range '{CONFIG_DATA_RANGE}'. Using default delivery/seasonal settings."
            )
        ...
        parsed_config["delivery"]["base_fee"] = _clean_currency(
            raw_key_values.get(
                "delivery base fee", parsed_config["delivery"]["base_fee"]
            )
        )
        parsed_config["delivery"]["per_mile_rate"] = _clean_currency(
            raw_key_values.get(
                "delivery per mile rate", parsed_config["delivery"]["per_mile_rate"]
            )
        )
        free_miles_raw = raw_key_values.get(
            "delivery free miles threshold",
            parsed_config["delivery"]["free_miles_threshold"],
        )
        try:
            parsed_config["delivery"]["free_miles_threshold"] = (
                int(free_miles_raw)
                if free_miles_raw is not None
                else parsed_config["delivery"]["free_miles_threshold"]
            )
        except (ValueError, TypeError):
            logfire.warning(
                f"Could not parse 'delivery free miles threshold': {free_miles_raw}. Using default: {parsed_config['delivery']['free_miles_threshold']}"
            )
        ...
        premium_rate = _clean_currency(
            raw_key_values.get("seasonal multiplier premium rate")
        )
        if premium_start_str and premium_end_str and premium_rate is not None:
            try:
                start_date = datetime.strptime(
                    str(premium_start_str), "%Y-%m-%d"
                ).date()
                ...
                logfire.warning(
                    f"Could not parse 'Premium' seasonal tier dates/rate from config: {e}"
                )
        ...
        pp_rate = _clean_currency(
            raw_key_values.get("seasonal multiplier premium plus rate")
        )
        if pp_start_str and pp_end_str and pp_rate is not None:
            try:
                ...
                logfire.warning(
                    f"Could not parse 'Premium Plus' seasonal tier dates/rate from config: {e}"
                )

        std_multiplier = _clean_currency(
            raw_key_values.get("seasonal multiplier standard")
        )
        ...
        if fetched_data["products"] is None or fetched_data["generators"] is None:
            ...
            try:
                ...
                if await self.redis_service.set_json(
                    ...
                    products_to_save_mongo = list(
                        pricing_catalog.get("products", {}).values()
                    )
                    ...
                    parsed_config_for_mongo = {
                        ...
                        "seasonal_multipliers_config": pricing_catalog.get(
                    ...
                    if (
                        mongo_product_sync_ok
                        and mongo_generator_sync_ok
                        and mongo_config_sync_ok
                    ):
    ...
    async def stop_background_sync(self):
        if self._sync_task and not self._sync_task.done():
            logfire.info("SheetSyncService: Attempting to stop background sync task...")
