...
class BlandAIManager:
    ...
    def __init__(
        self, api_key: str, base_url: str, pathway_id_setting: Optional[str] = None
    ):
    ...
    async def _sync_pathway(
            ...
            if mongo_service:
                ...
                if background_tasks:
                    background_tasks.add_task(
                        mongo_service.log_error_to_db, **error_details_config
                    )
                else:
        ...
        if not self.pathway_definition:
            ...
            if mongo_service:
                ...
                if background_tasks:
                    background_tasks.add_task(
                        mongo_service.log_error_to_db, **error_details_def
                    )
                else:
        ...
        if not pathway_name:
            ...
            if mongo_service:
                ...
                if background_tasks:
                    background_tasks.add_task(
                        mongo_service.log_error_to_db, **error_details_name
                    )
                else:
    ...
    async def _make_request(
        ...
        try:
            ...
            call_id_from_bland = (
                details.get("call_id") if isinstance(details, dict) else None
            )
    ...
    async def initiate_callback(
        ...
        if api_result.status == "error":
            ...
            background_tasks.add_task(
                mongo_service.update_bland_call_log_internal,
                contact_id=contact_id,
    ...
    async def _extract_data_from_transcript(
        self,
        transcript: list,
        mongo_service: MongoService,
        background_tasks: BackgroundTasks,
        ...
        try:
            ...
            if not transcript:
                ...
                error_message_content = (
                    "Transcript processing error: Received empty transcript."
                )
            ...
            logfire.info(
                f"Data extraction from transcript successful for contact_id: {contact_id}"
            )
            return BlandProcessingResult(
                success=True,
            ...
            error_message_content = (
                f"Unexpected error during transcript data extraction: {str(e)}"
            )
            logfire.error(
                f"{error_message_content} for contact_id: {contact_id}", exc_info=True
            )
            background_tasks.add_task(
                mongo_service.log_error_to_db,
                service_name="BlandAIManager._extract_data_from_transcript",
    ...
    async def process_incoming_transcript(
        ...
        if not contact_id:
            logfire.error(
                "Webhook received without contact_id in metadata.",
        ...
        logfire.info(
            f"Processing incoming transcript for contact_id: {contact_id}, Bland Call ID: {bland_call_id}"
        )
        ...
        processing_result = await self._extract_data_from_transcript(
            payload.transcript, mongo_service, background_tasks, contact_id
        )
        ...
        logfire.info(
            f"Background task scheduled to update Bland call log for contact_id: {contact_id} as COMPLETED."
        )

        # If Bland webhook itself indicates an error for this call
        if payload.error:
            logfire.warn(
                f"Bland webhook for contact_id: {contact_id}, call_id: {bland_call_id} reported an error: {payload.error}"
            )
            ...
            background_tasks.add_task(
                ...
                details={
                    ...
                    "webhook_payload_summary": payload.model_dump(
                        exclude={"transcript", "recording_url"}
                    ),  # Log key fields
                },
            )

    async def check_connection(self) -> Dict[str, Any]:
        """Checks the connection to the Bland API by fetching available prompts using self._client."""
        endpoint = "/v1/prompts"  # Changed endpoint
        log_url = f"{self.base_url.strip('/')}{endpoint}" # Corrected string stripping
        logfire.info(f"Checking Bland AI connection to {log_url} using httpx client by fetching prompts.")

        api_result: BlandApiResult = await self._make_request(method="GET", endpoint=endpoint)

        # For /v1/prompts, a successful Bland API response has "status": "success"
        # and a "prompts" key (which is an array) in its body.
        # The _make_request method already maps the Bland API's top-level "status" to api_result.status.
        if api_result.status == "success" and isinstance(api_result.details, dict) and "prompts" in api_result.details:
            logfire.info("Bland AI connection check successful via httpx (prompts endpoint).", details=api_result.details)
            return {"status": "success", "message": "Bland AI connection active (prompts listed).", "details": api_result.details}
        elif api_result.status == "success": # HTTP success, but "prompts" key missing or details not a dict
            logfire.warn(
                f"Bland AI connection check to /v1/prompts returned success status but 'prompts' key was missing or details structure was unexpected.",
                details=api_result.details
            )
            return {
                "status": "error",
                "message": "Bland AI connection issue: /v1/prompts responded successfully but 'prompts' key missing or unexpected details structure.",
                "details": api_result.details
            }
        else: # api_result.status is "error" (HTTP or network error, or Bland API returned non-success status)
            logfire.warn(
                f"Bland AI connection check failed (prompts endpoint): {api_result.message}",
                details=api_result.details
            )
            return {
                "status": "error",
                "message": f"Bland AI connection issue (prompts endpoint): {api_result.message}",
                "details": api_result.details
            }
...
async def sync_bland_pathway_on_startup(
    ...
    await bland_manager._sync_pathway(
        mongo_service=mongo_service, background_tasks=background_tasks
    )
