...
def _load_pathway_definition() -> Dict[str, Any]:
	"""
	...
	try:
		with open(PATHWAY_JSON_PATH, "r") as f:
			...
			knowledge_json_path = os.path.join(_PROJECT_ROOT, "app", "assets", "knowledge.json")
			try:
					...
					if "nodes" in pathway_data:
						for node in pathway_data["nodes"]:
							if node.get("type") == "Knowledge Base" and "data" in node:
								...
								logfire.info(f"Injected knowledge base content into node {node.get('id')}")
							...
							if node.get("type") == "Webhook" and "data" in node:
								...
								if "headers" in node["data"] and "Authorization" in node["data"]["headers"]:
									...
									logfire.info(f"Replaced hardcoded API key in node {node.get('id')}")

				logfire.info(f"Successfully loaded and injected knowledge base from {knowledge_json_path}")
...
def _load_location_tool_definition() -> Dict[str, Any]:
...
def _load_quote_tool_definition() -> Dict[str, Any]:
...
def _prepare_tool_json_data(tool_definition: Dict[str, Any]) -> Dict[str, Any]:
	"""
...
def _extract_data_from_transcript(
	...
	extracted_data["keywords"] = [keyword for keyword in important_keywords if keyword.lower() in full_text.lower()]
	...
	if extracted_data["contact_info"].get("name"):
		summary_parts.append(f"Contact: {extracted_data['contact_info'].get('name')}")

	if extracted_data["order_details"].get("product_type"):
		summary_parts.append(f"Product: {extracted_data['order_details'].get('product_type')}")

	if extracted_data["order_details"].get("event_type"):
		summary_parts.append(f"For: {extracted_data['order_details'].get('event_type')}")

	if extracted_data["order_details"].get("duration"):
		summary_parts.append(f"Duration: {extracted_data['order_details'].get('duration')} days")
...
class BlandAIManager:
	...
	def __init__(
		...
		self.pathway_definition = _load_pathway_definition()
		self.location_tool_definition = _load_location_tool_definition()
		self.quote_tool_definition = _load_quote_tool_definition()
	...
	async def _validate_pathway_config(self) -> bool:
		"""
	...
	async def _update_pathway_component(self, component_type: str, component_data: Dict[str, Any]) -> BlandApiResult:
		"""
	...
	async def _sync_pathway_nodes(self) -> None:
		...
		if not await self._validate_pathway_config():
			return
		...
		await self._update_pathway_component("nodes", update_payload)
	...
	async def _sync_pathway_edges(self) -> None:
		...
		await self._update_pathway_component("edges", update_payload)

	async def _validate_tool_definition(self, tool_type: str, tool_definition: Dict[str, Any], tool_id: str, json_path: str) -> bool:
		"""
		...
		if not tool_definition:
			logfire.error(
				f"Pathway: {tool_type} tool sync failed: Definition not loaded from {os.path.basename(json_path)}.")
			if self.mongo_service:
				error_details = {
					"service_name": f"BlandAIManager._sync_{tool_type.lower()}_tool",
					"error_type": f"{tool_type}ToolDefinitionError",
					"message": f"{tool_type} tool sync failed: Definition not loaded from {os.path.basename(json_path)}.",
	...
	async def _update_tool(self, tool_type: str, tool_id: str, json_data: Dict[str, Any]) -> BlandApiResult:
		"""
	...
	async def _sync_location_tool(self) -> None:
		...
		if not await self._validate_tool_definition(
			"Location", 
		...
		json_data = _prepare_tool_json_data(self.location_tool_definition)
		await self._update_tool("Location", self.location_tool_id, json_data)
	...
	async def _sync_quote_tool(self) -> None:
		...
		json_data = _prepare_tool_json_data(self.quote_tool_definition)
		await self._update_tool("Quote", self.quote_tool_id, json_data)
	...
	async def initiate_callback(
		...
		if self.mongo_service and self.background_tasks:
			...
			logfire.warn("BackgroundTasks not available for pre-call logging (will log synchronously if errors occur in _make_request).",
			             method="initiate_callback", contact_id=contact_id)
	...
	async def retry_call(
		...
		# mongo_service is used from self.mongo_service for fetching original log
	) -> BlandApiResult:
		"""
		...
		if not self.mongo_service:
			logfire.error(
				"Retry call failed: MongoService not available on BlandAIManager instance.", contact_id=contact_id)
		...
		# For mongo_service and background_tasks passed to retry_call are different and should be used by
	...
	async def process_incoming_transcript(
		...
		processing_result = _extract_data_from_transcript(payload.transcripts)
...
async def sync_bland_pathway_on_startup(mongo_service: MongoService):
	...
	await bland_manager._sync_bland()  # Call the sync method
