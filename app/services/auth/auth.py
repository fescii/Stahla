...
from bcrypt import checkpw, gensalt, hashpw  # Import bcrypt functions directly as a workaround for potential import issues
from jose import JWTError, jwt
from pydantic import EmailStr
import logfire
import uuid
from fastapi import Depends
from app.models.user import User, UserCreate, UserInDB, TokenData  # Ensure UserCreate is imported
from app.services.mongo.mongo import MongoService, get_mongo_service  # Import get_mongo_service
...
class AuthService:
        ...
        if settings.FIRST_SUPERUSER_EMAIL and settings.FIRST_SUPERUSER_PASSWORD:
            ...
            if not existing_user:
                ...
                if created_user:
                    ...
                    logfire.error("Failed to create initial superuser (see previous errors).")
            else:
                logfire.info("Initial superuser already exists.")
        else:
            logfire.info("Initial superuser email/password not configured in settings, skipping creation.")
    ...
    async def authenticate_user(self, email: EmailStr, password: str) -> Optional[UserInDB]:
        ...
        if not user:
            logfire.warning(f"Authentication failed: User '{email}' not found.") # Add log
            return None
        ...
        if not password_verified:
            logfire.warning(f"Authentication failed: Password verification failed for user '{email}'.") # Add log
...
async def startup_auth_service():
    ...
    try:
        ...
        logfire.error(f"Failed during Auth Service startup (initial user creation): {e}", exc_info=True)
