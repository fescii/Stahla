...
from app.models.hubspot import HubSpotDealProperties # Needed for pipeline/owner logic
from app.core.config import settings # Import settings for threshold
from app.services.classification_rules import classify_lead
from app.services.marvin_classification import marvin_classification_manager
...
class ClassificationManager:
    ...
    def _estimate_deal_value(self, input_data: ClassificationInput) -> float:
        ...
        if input_data.budget_mentioned:
            try:
                ...
                numeric_part = ''.join(c for c in input_data.budget_mentioned if c.isdigit() or c == '.')
                if numeric_part:
    ...
    async def classify_lead_data(self, input_data: ClassificationInput) -> ClassificationResult:
        ...
        """
        logfire.info("Starting lead classification.", input_source=input_data.source)
        ...
        if not input_data.intended_use and (input_data.lead_type_guess or input_data.event_type):
            source_text = (input_data.lead_type_guess or input_data.event_type or "").lower()
        ...
        if input_data.is_local is None:
            input_data.is_local = self._determine_locality(input_data.event_location_description)
        ...
        try:
            ...
            if settings.LLM_PROVIDER.lower() == "marvin" and settings.MARVIN_API_KEY:
                ...
                logfire.info("Using Marvin for lead classification", classification=classification)
            else:
                ...
                logfire.info("Using rule-based classification (Marvin not enabled)", classification=classification)
