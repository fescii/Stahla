...
class HubSpotManager:
    ...
    async def _handle_api_error(
        self, e: Exception, context: str, object_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if isinstance(e, (ObjectApiException, DealApiException)):
            ...
            logger.error(
                f"Unexpected error in {context}{f' for ID {object_id}' if object_id else ''}: {e}",
                exc_info=True,
            )
    ...
    async def get_pipelines(self, object_type: str) -> List[HubSpotPipeline]:
        ...
        try:
            ...
            api_response = self.client.crm.pipelines.pipelines_api.get_all(
    ...
    async def get_pipeline_stages(
        ...
        try:
            ...
            api_response = self.client.crm.pipelines.pipeline_stages_api.get_all(
    ...
    async def get_owners(self, limit: int = 100) -> List[HubSpotOwner]:
        ...
        try:
            while True:
                api_response = self.client.crm.owners.owners_api.get_page(
    ...
    async def associate_objects(
        ...
        try:
            ...
            if hasattr(api_response, "status") and api_response.status == "COMPLETE":
                ...
                logger.error(
                    f"Failed to {context}. Status: {api_response.status}, Errors: {api_response.errors if hasattr(api_response, 'errors') else 'N/A'}"
                )
                return False
            else:  # FAILED or other status
                logger.error(
                    f"Failed to {context}. Status: {api_response.status if hasattr(api_response, 'status') else 'Unknown'}, Response: {api_response}"
                )
                return False
    ...
    async def batch_associate_objects(self, inputs: List[Dict[str, Any]]) -> bool:
        ...
        for item in inputs:
            try:
                from_obj_type_norm = item[
                    "from_object_type"
                ].upper()  # Normalize: "contacts" -> "CONTACTS"
    ...
    async def create_lead(
    ...
    ) -> Dict[str, Any]:  # Changed LeadCreateSchema to HubSpotLeadInput
        logger.info(
        ...
        try:
            ...
            if not response["errors"] and (contact_id or company_id or deal_id):
                ...
                response["success"] = (
                    True  # Or False, depending on if "nothing created" is an error
                )
                response["message"] = (
                    "Lead data processed, but no new HubSpot entities were created (e.g., contact/company might have existed and were not updated, or data was insufficient)."
                )
                ...
                response["message"] = (
                    f"Lead processing encountered errors: {'; '.join(final_error_messages)}"
                )
            ...
            ValidationError
        ) as ve:  # Pydantic validation error for input LeadCreateSchema
            logger.error(
