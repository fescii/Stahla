...
class HubSpotManager:
    ...
    async def _make_request(
        ...
        logfire.debug(f"Making HubSpot API request: {method} {url}", params=params, data=json_data)
        try:
            ...
            logfire.error(f"HubSpot API HTTP error: {e.response.status_code}", url=str(e.request.url), response=e.response.text)
            ...
            logfire.error(f"Unexpected error during HubSpot API request: {e}", exc_info=True)
    ...
    async def search_contact_by_email(self, email: str) -> Optional[str]:
        ...
        payload = {
            ...
            "properties": ["email"], # Only need ID, but request at least one property
            "limit": 1
        }
        ...
        if result.status == "success" and result.details.get("total", 0) > 0:
            ...
            logfire.info(f"Found existing HubSpot contact by email.", email=email, contact_id=contact_id)
            return contact_id
    ...
    async def create_or_update_contact(self, contact_data: HubSpotContactProperties) -> HubSpotContactResult:
        ...
        if existing_contact_id:
            ...
            logfire.info("Updating existing HubSpot contact.", contact_id=existing_contact_id, email=contact_data.email)
            ...
            if result.status == "success":
                return HubSpotContactResult(status="success", message="Contact updated successfully.", contact_id=existing_contact_id, properties=result.details.get("properties", {}))
            else:
            ...
            if result.status == "success":
                ...
                return HubSpotContactResult(status="success", message="Contact created successfully.", contact_id=new_contact_id, properties=result.details.get("properties", {}))
            else:
                return HubSpotContactResult(status="error", message=f"Failed to create contact: {result.message}", details=result.details)
    ...
    async def create_deal(self, deal_data: HubSpotDealProperties, associated_contact_id: Optional[str] = None) -> HubSpotDealResult:
        ...
        if result.status == "success":
            ...
            logfire.info("HubSpot deal created successfully.", deal_id=deal_id, deal_name=deal_data.dealname)
            return HubSpotDealResult(status="success", message="Deal created successfully.", deal_id=deal_id, properties=result.details.get("properties", {}))
        else:
            logfire.error("Failed to create HubSpot deal.", deal_name=deal_data.dealname, error=result.message)
            return HubSpotDealResult(status="error", message=f"Failed to create deal: {result.message}", details=result.details)
    ...
    async def update_deal_pipeline_and_owner(self, deal_id: str, pipeline_id: str, stage_id: str, owner_id: Optional[str] = None) -> HubSpotApiResult:
        ...
        """
        logfire.info(f"Updating pipeline/stage/owner for deal {deal_id}.", pipeline=pipeline_id, stage=stage_id, owner=owner_id)
        ...
        if result.status == "success":
            logfire.info(f"Deal {deal_id} pipeline/stage/owner updated successfully.")
        else:
            logfire.error(f"Failed to update pipeline/stage/owner for deal {deal_id}.", error=result.message)
    ...
    async def get_pipeline_id(self, pipeline_name: str, object_type: str = "deals") -> Optional[str]:
        ...
        logfire.info(f"Fetching {object_type} pipelines to find ID for '{pipeline_name}'.")
        ...
        if result.status == "success" and "results" in result.details:
            for pipeline in result.details["results"]:
                if pipeline.get("label", "").lower() == pipeline_name.lower():
                    ...
                    logfire.info(f"Found pipeline ID.", name=pipeline_name, id=pipeline_id)
                    return pipeline_id
            logfire.warn(f"Pipeline not found by name.", name=pipeline_name)
            return None
        else:
            logfire.error("Failed to fetch pipelines from HubSpot.", error=result.message)
            return None
    ...
    async def get_stage_id(self, pipeline_id: str, stage_name: str, object_type: str = "deals") -> Optional[str]:
        ...
        logfire.info(f"Fetching stages for pipeline {pipeline_id} to find ID for stage '{stage_name}'.")
        result = await self._make_request("GET", endpoint)
        ...
        if result.status == "success" and "results" in result.details:
            for stage in result.details["results"]:
                if stage.get("label", "").lower() == stage_name.lower():
                    ...
                    logfire.info(f"Found stage ID.", pipeline_id=pipeline_id, name=stage_name, id=stage_id)
                    return stage_id
            logfire.warn(f"Stage not found by name in pipeline.", name=stage_name, pipeline_id=pipeline_id)
            return None
        else:
            logfire.error(f"Failed to fetch stages for pipeline {pipeline_id} from HubSpot.", error=result.message)
            return None
    ...
    async def get_owners(self, email: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        if result.status == "success" and "results" in result.details:
            ...
            logfire.error("Failed to fetch owners from HubSpot.", error=result.message)
            return []
    ...
    async def _update_owner_list_if_needed(self):
        ...
        if not self._owner_list or not self._owners_last_fetched or (now - self._owners_last_fetched > self._owner_cache_ttl):
            ...
            if self._owner_list:
                ...
                logfire.info(f"Initialized round-robin with {len(self._owner_list)} owners.")
            else:
    ...
    async def get_next_owner_id(self, team_name: Optional[str] = None) -> Optional[str]:
        ...
        async with self._owner_lock: # Ensure atomic update of iterator
            ...
            if not self._owner_iterator:
                logfire.error("Owner iterator not initialized, cannot assign next owner.")
                return None
            ...
            if team_name:
                logfire.warn("Filtering owners by team_name is not implemented in get_next_owner_id.")

            try:
                next_owner = next(self._owner_iterator)
                owner_id = next_owner.get("id")
                logfire.info(f"Assigned next owner via round-robin.", owner_id=owner_id, owner_email=next_owner.get("email"))
                return owner_id
