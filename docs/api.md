...
**Authentication:** Specific endpoints require authentication as noted.
    *   **Pricing Webhooks:** Require an API key passed via the `Authorization: Bearer <API_KEY>` header, matching the `PRICING_WEBHOOK_API_KEY` setting.
    *   **Dashboard API:** Requires authentication (placeholder implementation currently allows access).
...
*   **`POST /hubspot`**
    *   **Description:** Endpoint intended to receive webhook events *from* HubSpot (e.g., contact updates). Placeholder implementation.
...
*   **`POST /pricing/quote`**
    *   **Description:** Calculates a real-time price quote based on trailer type, duration, usage, extras, and delivery location. Relies on cached pricing data (from Google Sheets) and cached location data (from `/location_lookup` or calculated on demand). Logs request/response data via background task.
    *   **Auth:** Required (`Authorization: Bearer <PRICING_WEBHOOK_API_KEY>`)
    *   **Request Body:** `QuoteRequest` model (defined in `app.models.quote`). Contains `request_id`, `delivery_location`, `trailer_type`, `rental_start_date`, `rental_days`, `usage_type`, `event_tier`, `extras`.
    *   **Response Body:** `QuoteResponse` model (defined in `app.models.quote`). Contains `request_id`, `quote_id`, and a nested `quote` object with `line_items`, `subtotal`, `delivery_tier_applied`, `notes`. (Status 200 OK on success, 400/500 on error).
